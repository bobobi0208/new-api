...
export function ChannelMutateDrawer({
  ...
  const onSubmit = useCallback(
    async (data: ChannelFormValues) => {
      ...
      // Persist reconciliation upstream_type for this channel as a
      // best-effort side-effect. Failures here must not block the main
      // create/update flow, so they only surface as warnings.
      try {
        if (isEditing && currentRow) {
          ...
          const existingCfg = (reconciliationConfigsData?.data || []).find(
            (c) => c.channel_id === currentRow.id
          )
        ...
        toast.warning(
          t('Failed to save reconciliation type: {{msg}}', {
            msg:
              (reconcErr instanceof Error && reconcErr.message) ||
              'unknown error',
          })
        )
      }
                ...
                <>
                  ...
                  <div className={sideDrawerSectionClassName()}>
                    ...
                    >
                      <SelectTrigger
                        className={
                          upstreamTypeError ? 'border-destructive' : undefined
                        }
                      >
