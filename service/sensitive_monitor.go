...
type SensitiveMonitorInput struct {
	...
	Path       string
	PromptText string
	// Role 标识 PromptText 的来源角色（"user" / "assistant" / "system" / ""）。
	// assistant 角色命中 Block 时降级为 Monitor，避免模型输出违规直接禁用用户令牌。
	Role string
}
...
// sensitiveNegationKeywords 是命中区间前后窗口内出现即视为"非有害请求"的语境关键词，
// 用于跳过受害者陈述 / 法律讨论 / 举报 / 反对类语境的命中。仅对 Monitor 动作生效，
// Block 动作的高危词不走豁免。
var sensitiveNegationKeywords = []string{
	"反对", "抵制", "严禁", "禁止", "严打", "打击", "制止", "违法", "犯罪",
	"指控", "控诉", "控告", "被告", "被指控", "举报", "告发", "报警", "报案", "谴责", "批评",
	"反抗", "受害", "受害者", "遭遇", "遭到", "幸存者",
	"法律", "刑法", "条例", "判决", "量刑",
	"oppose", "prohibit", "forbidden", "illegal", "crime", "accuse", "report", "victim", "survivor",
}
...
func CheckSensitiveMonitor(input SensitiveMonitorInput) (SensitiveMonitorResult, error) {
	...
	now := time.Now()
	assistantRole := input.Role == "assistant"
	for _, rule := range rules {
		...
		if assistantRole && effectiveAction == model.SensitiveWordActionBlock {
			effectiveAction = model.SensitiveWordActionMonitor
		}
		if effectiveAction == model.SensitiveWordActionMonitor && contextHasNegation(input.PromptText, matchStart, matchEnd) {
			continue
		}
		result.Matched = true
		if effectiveAction == model.SensitiveWordActionBlock {
			result.Blocked = true
		}
...
// contextHasNegation 检查命中区间前后 sensitiveNegationContextRunes 个字符（rune）窗口内
// 是否出现否定 / 陈述 / 控诉 / 法律语境关键词。出现即视为非有害请求（如受害者陈述、
// 法律讨论、举报场景），调用方应跳过本次命中以减少误杀。前缀和后缀分别扫描，
// 否定词必须完整出现在一侧。
func contextHasNegation(text string, matchStart, matchEnd int) bool {
	const sensitiveNegationContextRunes = 16
	if matchStart < 0 || matchEnd < matchStart || matchStart > len(text) {
		return false
	}
	if matchEnd > len(text) {
	...
	prefix := []rune(text[:matchStart])
	suffix := []rune(text[matchEnd:])
	if len(prefix) > sensitiveNegationContextRunes {
		prefix = prefix[len(prefix)-sensitiveNegationContextRunes:]
	}
	if len(suffix) > sensitiveNegationContextRunes {
		suffix = suffix[:sensitiveNegationContextRunes]
	}
	prefixLower := strings.ToLower(string(prefix))
	suffixLower := strings.ToLower(string(suffix))
	for _, keyword := range sensitiveNegationKeywords {
		kw := strings.ToLower(keyword)
		if strings.Contains(prefixLower, kw) || strings.Contains(suffixLower, kw) {
			return true
		}
	}
	return false
...
func SeedDefaultSensitiveWords() (SensitiveWordsSeedResult, error) {
	defaults := []model.SensitiveWord{
		...
		{Pattern: `(?i)(强|轮)\s*奸\s*(儿童|幼女|幼童|小学生|未成年)`, IsRegex: true, Enabled: true, Action: model.SensitiveWordActionMonitor, Description: `【中文正则·组合】强制性侵害动词 + 必选未成年人类词（容忍中间空格）；裸词"强奸"由其它规则与上下文豁免兜底`},
